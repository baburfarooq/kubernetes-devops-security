pipeline {
  ...
  stages {
    ...
    stage('SonarQube SAST') {
      steps {
        withSonarQubeEnv('SonarQube') {
              sh "mvn sonar:sonar -Dsonar.projectKey=numeric-application -Dsonar.host.url=http://devsecops-demo.centralus.cloudapp.azure.com:9000"
        }
    ...
    stage ('Vulnerability Scan - Docker ') {
      ...
      post {
        always {
          dependencyCheckPublisher pattern: 'target/dependency-check-report.xmI'
